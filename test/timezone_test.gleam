import gleam/bit_array
import gleam/time/calendar
import gleam/time/timestamp
import gleeunit
import timezone
import timezone/internal

const tzsample = ""

const tzsample2 = "VFppZjIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQAAAAQAAAAAAABVVEMAVFppZjIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAQAAAAQAAAAAAABVVEMAClVUQzAK"

pub fn main() -> Nil {
  gleeunit.main()
}

// gleeunit test functions end in `_test`
pub fn hello_world_test() {
  let name = "Joe"
  let greeting = "Hello, " <> name <> "!"

  assert greeting == "Hello, Joe!"
}

pub fn path_test() {
  assert timezone.tzfile_path("UTC") == "/usr/share/zoneinfo/UTC"
}

pub fn null_split_test() {
  let ba = <<"hello":utf8, 0:int-size(8), "world":utf8>>
  let first = internal.split_at_null(ba, 0)

  let second = internal.split_at_null(<<"world":utf8>>, 0)

  assert first == Ok(#(<<"hello":utf8>>, <<"world":utf8>>))
  assert second == Error(Nil)
}

pub fn parse_simple_test() {
  let assert Ok(tzdata) = bit_array.base64_decode(tzsample2)
  let assert Ok(tz) = internal.parse(tzdata)

  assert tz.header == internal.TimeZoneHeader(2, 0, 0, 0, 0, 1, 4)
  assert tz.fields.ttinfos == [internal.TTInfo(0, 0, 0)]
  assert tz.fields.designations == ["UTC"]
}

pub fn parse_components_test() {
  let assert Ok(tzdata) = bit_array.base64_decode(tzsample)
  let assert Ok(tz) = internal.parse(tzdata)

  assert tz.header == internal.TimeZoneHeader(2, 6, 6, 0, 236, 6, 20)
  assert tz.fields.ttinfos
    == [
      internal.TTInfo(-17_762, 0, 0),
      internal.TTInfo(-14_400, 1, 4),
      internal.TTInfo(-18_000, 0, 8),
      internal.TTInfo(-18_000, 0, 8),
      internal.TTInfo(-14_400, 1, 12),
      internal.TTInfo(-14_400, 1, 16),
    ]
  assert tz.fields.designations == ["LMT", "EDT", "EST", "EST", "EWT", "EPT"]
}

pub fn test_get_time_in_local_zone() {
  let assert Ok(tzdata) = bit_array.base64_decode(tzsample)
  let ts = timestamp.from_unix_seconds(1_758_086_280)
  assert timezone.get_time_with_tzdata(ts, tzdata)
    == Ok(timezone.TimeInZone(
      calendar.Date(2025, calendar.September, 17),
      calendar.TimeOfDay(1, 18, 0, 0),
      "EDT",
      True,
    ))
}

pub fn test_get_time_in_utc_zone() {
  let assert Ok(tzdata) = bit_array.base64_decode(tzsample2)
  let ts = timestamp.from_unix_seconds(1_758_086_280)
  assert timezone.get_time_with_tzdata(ts, tzdata)
    == Ok(timezone.TimeInZone(
      calendar.Date(2025, calendar.September, 17),
      calendar.TimeOfDay(5, 18, 0, 0),
      "UTC",
      False,
    ))
}
